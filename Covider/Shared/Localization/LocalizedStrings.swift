//
//  LocalizedStrings.swift
//  Covider
//
//  Created by Jakub "GPH4PPY" Dąbrowski on 22/08/2021.
//

import SwiftUI

struct LocalizedStrings {
    // MARK: - Welcome View
    public static let welcomeToCovider = NSLocalizedString("welcome_to_covider", comment: "")
    public static let startUsingApp = NSLocalizedString("start_using_app", comment: "")
    public static let navigation = NSLocalizedString("navigation", comment: "")
    public static let thePurpose = NSLocalizedString("the_purpose", comment: "")
    public static let headInTheClouds = NSLocalizedString("head_in_the_clouds", comment: "")
    public static let youAreInControl = NSLocalizedString("you_are_in_control", comment: "")
    public static let ready = NSLocalizedString("ready", comment: "")
    public static let welcomeScreenDescription1 = NSLocalizedString("welcome_screen_description_1", comment: "")
    public static let welcomeScreenDescription2 = NSLocalizedString("welcome_screen_description_2", comment: "")
    public static let welcomeScreenDescription3 = NSLocalizedString("welcome_screen_description_3", comment: "")
    public static let welcomeScreenDescription4 = NSLocalizedString("welcome_screen_description_4", comment: "")
    public static let welcomeScreenDescription5 = NSLocalizedString("welcome_screen_description_5", comment: "")
    public static let emptyList = NSLocalizedString("empty_list", comment: "")
    
    // MARK: - Home View
    public static let duties = NSLocalizedString("duties", comment: "")
    public static let newDuty = NSLocalizedString("new_duty", comment: "")

    // MARK: - Session View
    public static let endSession = NSLocalizedString("end_session", comment: "")
    public static let confirm = NSLocalizedString("confirm", comment: "")
    public static let endSessionDescription = NSLocalizedString("end_session_description", comment: "")
    public static let goBack = NSLocalizedString("go_back", comment: "")
    public static let peopleInThisPlace = NSLocalizedString("people_in_this_place", comment: "")

    // MARK: - Detail View
    public static let allPeople = NSLocalizedString("all_people", comment: "")
    public static let guardString = NSLocalizedString("guard", comment: "")
    public static let startDate = NSLocalizedString("start_date", comment: "")
    public static let endDate = NSLocalizedString("end_date", comment: "")
    public static let vaccinated = NSLocalizedString("vaccinated", comment: "")
    public static let unvaccinated = NSLocalizedString("unvaccinated", comment: "")
    
    // MARK: - Duty Setup Alert
    public static let setupDutyTitle = NSLocalizedString("setup_duty_title", comment: "")
    public static let setupDutyDescription = NSLocalizedString("setup_duty_description", comment: "")
    public static let title = NSLocalizedString("title", comment: "")
    public static let guardName = NSLocalizedString("guard_name", comment: "")
    public static let place = NSLocalizedString("place", comment: "")
    public static let vaccinatedDivision = NSLocalizedString("vaccinated_division", comment: "")
    public static let cancel = NSLocalizedString("cancel", comment: "")
    public static let startDuty = NSLocalizedString("start_duty", comment: "")
}
